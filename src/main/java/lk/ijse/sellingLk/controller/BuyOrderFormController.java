...
public class BuyOrderFormController {
    ...
    private BuyOrderBO buyOrderBO= (BuyOrderBO) BOFactory.getBoFactory().getBO(BOFactory.BOTypes.BUYORDER);
...
//        lblDate.setText(LocalDate.now().toString());
    ...
    private void loadVehicleIds() {
        ...
        try {
            List<String> allVehicle = vehicleBO.getNotGetVehicle();
    ...
    void btnItemIdOnAction(ActionEvent event) {
        try {
            dto = buyOrderBO.getVehicleInfo(cmbItemId.getValue());
    ...
    private void calculateTotal() {
        cart.forEach(id -> {
            try {
                VehicleDto vehicleInfo = buyOrderBO.getVehicleInfo(id);
    ...
    void btnPlaceOrderOnAction(ActionEvent event) {
        ...
        if (ValidateUtil.validatePhone(txtSellerContact.getText(),txtSellerContact)) {
            try {
                SellerDto sellerInfo = new SellerBOImpl().getSellerInfo(txtSellerContact.getText());
                id = buyOrderBO.generateNextOrderId();
                ...
                boolean isSaved = buyOrderBO.saveBuyOrder(pdto);
                if (isSaved) {
                    generateReport(pdto);
                    String email = new SellerBOImpl().getEmail(pdto.getCusId());
    ...
    void txtSellerContactOnAction(ActionEvent event) {
        try {
            SellerDto info = new SellerBOImpl().getSellerInfo(txtSellerContact.getText());
            if (info != null) {
