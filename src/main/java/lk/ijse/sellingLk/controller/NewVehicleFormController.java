...
public class NewVehicleFormController {
    ...
    private VehicleBO vehicleBO = (VehicleBO) BOFactory.getBoFactory().getBO(BOFactory.BOTypes.VEHICLE);
    ...
    void btnNewVehicleOnAction(ActionEvent event) {
        ...
        if (ValidateUtil.validateVehicleNumber(number, txtNumber) &&
            ...
            try {
                ...
                boolean isSaved = vehicleBO.saveVehicle(new VehicleDto(vehicleBO.getNextVehicleId(), type, brand, model, Integer.parseInt(year), fuelType, Integer.parseInt(engineCapacity), Integer.parseInt(mileage), number, Integer.parseInt(price), "On Hand"));
                if (isSaved) {
